...
| `esp32_camera_set_brightness` | Statement | BRIGHTNESS(field_number), CONTRAST(field_number), SATURATION(field_number) | `esp32_camera_set_brightness(0, 0, 0)` | { sensor_t * s = esp_camera_sensor_get(); s->set_brightness(s, ...); s->set_contrast(s, .. |
...
| `esp32_camera_release` | Statement | FRAME(input_value) | `esp32_camera_release(math_number(0))` | if(...) { camera_fb_t* _fb = (camera_fb_t*)...; // 额外的安全检查 if(_fb->buf && _fb->len > 0) { |
| `esp32_camera_send_serial` | Statement | FRAME(input_value) | `esp32_camera_send_serial(math_number(0))` | camera_fb_t* _fb = (camera_fb_t*)...; if(_fb) { if(_fb->buf && _fb->len > 0) { // 发送帧开始标识 |
