...
| `ds1302_set_write_protect` | Statement | VAR(field_variable), ENABLE(field_dropdown) | `ds1302_set_write_protect($rtc, TRUE)` | `rtc.writeProtect(true);` |
| `ds1302_set_halt` | Statement | VAR(field_variable), HALT(field_dropdown) | `ds1302_set_halt($rtc, FALSE)` | `rtc.halt(false);` |
| `ds1302_get_time` | Value | VAR(field_variable) | `ds1302_get_time($rtc)` | `rtc.time()` |
| `ds1302_set_time` | Statement | VAR(field_variable), YEAR(input_value), MONTH(input_value), DAY(input_value), HOUR(input_value), MINUTE(input_value), SECOND(input_value), WEEKDAY(input_value) | `ds1302_set_time($rtc, math_number(2024), math_number(1), math_number(1), math_number(12), math_number(0), math_number(0), math_number(2))` | `Time time_rtc(2024, 1, 1, 12, 0, 0, (Time::Day)2); rtc.time(time_rtc);` |
...
| `ds1302_write_ram` | Statement | VAR(field_variable), ADDRESS(input_value), VALUE(input_value) | `ds1302_write_ram($rtc, math_number(0), math_number(100))` | `rtc.writeRam(0, 100);` |
| `ds1302_read_ram` | Value | VAR(field_variable), ADDRESS(input_value) | `ds1302_read_ram($rtc, math_number(0))` | `rtc.readRam(0)` |
