...
| `esp_now_send` | Statement | VAR(field_variable), MESSAGE(input_value) | `esp_now_send(variables_get($peer1), text("hello"))` | `if (` |
...
| `esp_now_on_receive` | Statement | VAR(field_variable) | `esp_now_on_receive(variables_get($peer1))` | `` |
| `esp_now_on_sent` | Statement | VAR(field_variable) | `esp_now_on_sent(variables_get($peer1))` | `` |
...
| `esp_now_serial_read_string_until` | Value | VAR(field_variable), TERMINATOR(input_value) | `esp_now_serial_read_string_until(variables_get($nowSerial), math_number(0))` | — |
...
1. **Variable Creation**: `esp_now_create_peer("varName", ...)` creates variable `$varName`; reference with `variables_get($varName)`
