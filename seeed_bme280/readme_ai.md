...
| `bme280_get_temperature` | Value | VAR(field_variable) | `bme280_get_temperature(variables_get($name))` | `name.getTemperature()` |
| `bme280_get_pressure` | Value | VAR(field_variable) | `bme280_get_pressure(variables_get($name))` | `name.getPressure()` |
| `bme280_get_humidity` | Value | VAR(field_variable) | `bme280_get_humidity(variables_get($name))` | `name.getHumidity()` |
| `bme280_calc_altitude` | Value | VAR(field_variable), PRESSURE(input_value) | `bme280_calc_altitude(variables_get($name), math_number(101325))` | `name.calcAltitude(101325)` |
