...
| `mfrc522_when_card_detected` | Statement | VAR(field_variable) | `mfrc522_when_card_detected(variables_get($rfid))` | `` |
| `mfrc522_authenticate` | Statement | VAR(field_variable), SECTOR(input_value), KEY_TYPE(dropdown), KEY(input_value) | `mfrc522_authenticate(variables_get($rfid), math_number(0), A, math_number(0))` | (dynamic code) |
...
| `mfrc522_write_block` | Statement | VAR(field_variable), DATA(input_value), BLOCK(input_value) | `mfrc522_write_block(variables_get($rfid), math_number(0), math_number(0))` | (dynamic code) |
...
1. **Variable Creation**: `mfrc522_setup("varName", ...)` creates variable `$varName`; reference with `variables_get($varName)`
