...
| `rs485_simple_receive` | Statement | DO(input_statement) | `rs485_simple_receive() @DO: child_block()` | if (RS485.available()) { rs485_receivedData = ""; while (RS485.available()) { char c = RS4 |
| `rs485_received_data` | Value | (none) | `rs485_received_data()` | rs485_receivedData |
| `rs485_begin` | Statement | SERIAL(dropdown), BAUDRATE(dropdown), TX_PIN(dropdown), DE_PIN(dropdown), RE_PIN(dropdown) | `rs485_begin(SERIAL, BAUDRATE, TX_PIN, DE_PIN, RE_PIN)` | Dynamic code |
...
| `rs485_slave_receive` | Statement | SLAVE_ADDR(field_number), DO(input_statement) | `rs485_slave_receive(1) @DO: child_block()` | if (RS485.available()) { rs485_receivedData = ""; while (RS485.available()) { char c = RS4 |
