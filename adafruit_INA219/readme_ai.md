...
| `ina219_read_value` | Value | VAR(field_variable), TYPE(dropdown) | `ina219_read_value($ina219, BUS_VOLTAGE)` | (dynamic code) |
...
1. **Variable Creation**: `ina219_init_with_wire("varName", ...)` creates variable `$varName`; reference with `$varName`
