...
| `tca8418_when_key_event` | Statement | VAR(field_variable) | `tca8418_when_key_event(variables_get($keypad))` | `` |
...
arduino_setup()
    ...
    tca8418_begin(variables_get($keypad), math_number(0))
...
1. **Variable Creation**: `tca8418_create("varName", ...)` creates variable `$varName`; reference with `variables_get($varName)`
