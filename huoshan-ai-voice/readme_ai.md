...
arduino_setup()
    ...
    huoshan_ai_mic_config(I2S_NUM_1, math_number(42), math_number(2), math_number(1), math_number(8), math_number(15))
...
6. **Streaming behavior**: `huoshan_ai_start_listening` starts recording and opens a streaming ASR task. `huoshan_ai_stop_listening_and_chat` finalizes ASR, sends the recognized text to Coze, and streams TTS playback by sentence.
...
8. **Diagnostics**: serial logs include state changes, I2S pin readiness, recorded peak/average audio levels, ASR streaming timing, and TTS connection timing. Use `huoshan_ai_get_last_error()` for the latest runtime error.
9. **Common ABS error**: do not write bare strings for input_value slots. Use `text("ssid")`, `math_number(20000)`, and dropdown enum values exactly as listed.
