...
| `sscma_check_last_image` | Value | VAR(field_variable) | `sscma_check_last_image(variables_get($ai))` | — |
| `sscma_get_last_image` | Value | VAR(field_variable) | `sscma_get_last_image(variables_get($ai))` | — |
...
1. **Variable Creation**: `sscma_begin_i2c("varName", ...)` creates variable `$varName`; reference with `variables_get($varName)`
