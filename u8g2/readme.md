...
|--------|------|----------|----------|----------|
| `u8g2_begin` | 语句块 | TYPE(field_dropdown), MODE(field_dropdown), RESOLUTION(动态), PROTOCOL(动态) + 动态引脚输入 | `"fields":{"TYPE":"SSD1306","MODE":"FULL_BUFFER","RESOLUTION":"128X64_NONAME","PROTOCOL":"_HW_I2C","RESET_PIN":"U8X8_PIN_NONE"}` | 声明 `U8G2_*` 对象并在 `setup()` 调用 `u8g2.begin();`（generator 会 `addLibrary('#include <U8g2lib.h>')` 并 `addObject('u8g2', 'U8G2_... u8g2(...);')`） |
| `u8g2_page_buffer` | 语句块（包裹） | DO(input_statement) | `{ "type": "u8g2_page_buffer", "inputs": { "DO": { "block": {...} } } }` | `u8g2.firstPage(); do { ... } while (u8g2.nextPage());` |
...
| `u8g2_draw_pixel` | 语句块 | X(input_value Number), Y(input_value Number) | `"inputs":{"X":{"block":{"type":"math_number","fields":{"NUM":"10"}}},"Y":{...}}` | `u8g2.drawPixel(x, y);`（自动判断是否添加 `sendBuffer()`） |
| `u8g2_draw_line` | 语句块 | X1/Y1/X2/Y2 (input_value Number) | — | `u8g2.drawLine(x1,y1,x2,y2);`（自动判断是否添加 `sendBuffer()`） |
| `u8g2_draw_rectangle` | 语句块 | X/Y/WIDTH/HEIGHT (input_value Number), FILL(field_checkbox) | — | `u8g2.drawBox(...)`（填充）或 `u8g2.drawFrame(...)`（空心） |
| `u8g2_draw_circle` | 语句块 | X/Y/RADIUS (input_value Number), FILL(field_checkbox) | — | `u8g2.drawDisc(...)` 或 `u8g2.drawCircle(...)` |
| `u8g2_draw_str` | 语句块 | X/Y (input_value Number), TEXT (input_value String/Number) | — | `u8g2.drawUTF8(x, y, text);`（generator 会在需要时调用 `u8g2.enableUTF8Print()`） |
...
| `u8g2_draw_bitmap` | 语句块 | X/Y (input_value Number), BITMAP(input_value Bitmap) | — | `u8g2.drawXBMP(x,y,width,height, var_data);` |
...
| `u8g2_set_font_mode` | 语句块 | MODE(field_dropdown) | `"fields":{"MODE":"1"}` | `u8g2.setFontMode(...);` |
...
```c
u8g2.drawUTF8(10, 20, "Hello");
u8g2.sendBuffer();  // 在 Full Buffer 模式下自动添加
```
...
2. **两种绘制模式**:
   - **Full Buffer 模式**: 使用 `u8g2_clear_buffer()` 清空缓冲区，绑定多个绘制操作后，通过 `u8g2_send_buffer()` 一次性发送到显示器。绘制块会自动判断是否需要添加 `sendBuffer()`。
   - **Page Buffer 模式**: 使用 `u8g2_page_buffer` 块包裹绘制操作，在其内部的绘制块不会自动添加 `sendBuffer()`，由 page buffer 循环自动处理刷新。
...
- **MODE(缓冲模式)**: "FULL_BUFFER"（全缓冲区）, "PAGE_BUFFER"（页面缓冲区）
- **RESOLUTION**: 根据 TYPE 动态变化，如 "128X64_NONAME", "128X32_UNIVISION", "64X128" 等
- **PROTOCOL**: `_HW_I2C`, `_SW_I2C`, `_3W_HW_SPI`, `_3W_SW_SPI`, `_4W_HW_SPI`, `_4W_SW_SPI`
- **FONT**: 常见的 `u8g2` 字体常量（如 `u8g2_font_ncenB08_tr`, `u8g2_font_wqy12_t_chinese2` 等）
