...
| `rfid_on_tag_read` | Statement | VAR(field_variable) | `rfid_on_tag_read(variables_get($rfidReader))` | `` |
...
1. **Variable Creation**: `rfid_setup("varName", ...)` creates variable `$varName`; reference with `variables_get($varName)`
