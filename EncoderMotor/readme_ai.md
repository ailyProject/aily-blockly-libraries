...
|------------|------------|--------------------------|------------|----------------|
| `encoder_motor_create` | Statement | VAR(field_input), POS_PIN(input_value), NEG_PIN(input_value), A_PIN(input_value), B_PIN(input_value), PPR(input_value), REDUCTION(input_value), PHASE(dropdown) | `encoder_motor_create("motor1", math_number(2), math_number(2), math_number(2), math_number(2), math_number(0), math_number(0), em::EncoderMotor::kAPhaseLeads)` | Dynamic code |
...
1. **Variable**: `encoder_motor_create("varName", ...)` creates variable `$varName`; reference it later with `variables_get($varName)`.
