...
|------------|------------|--------------------------|------------|----------------|
| `esp32_i2s_create` | Statement | VAR(field_input) | `esp32_i2s_create("i2s")` | Dynamic code |
...
| `esp32_i2s_generate_tone` | Statement | VAR(field_variable), FREQUENCY(input_value), DURATION(input_value), AMPLITUDE(input_value) | `esp32_i2s_generate_tone(variables_get($i2s), math_number(0), math_number(1000), math_number(0))` | i2s_generate_tone(..., ..., ..., ...);\n |
...
1. **Variable**: `esp32_i2s_create("varName", ...)` creates variable `$varName`; reference it later with `variables_get($varName)`.
