...
| `rp_ble_adv_get_address` | Value | ADV_VAR(field_variable) | `rp_ble_adv_get_address($bleAdv)` | (dynamic code) |
| `rp_ble_adv_get_rssi` | Value | ADV_VAR(field_variable) | `rp_ble_adv_get_rssi($bleAdv)` | (dynamic code) |
| `rp_ble_adv_is_ibeacon` | Value | ADV_VAR(field_variable) | `rp_ble_adv_is_ibeacon($bleAdv)` | (dynamic code) |
| `rp_ble_adv_name_has_prefix` | Value | ADV_VAR(field_variable), PREFIX(input_value) | `rp_ble_adv_name_has_prefix($bleAdv, math_number(0))` | (dynamic code) |
...
| `rp_ble_write_buffer` | Statement | BUFFER_VAR(field_variable), DATA(input_value) | `rp_ble_write_buffer($buffer, math_number(0))` | `if (` |
...
| `rp_ble_connect_adv` | Statement | ADV_VAR(field_variable), TIMEOUT(input_value) | `rp_ble_connect_adv($bleAdv, math_number(1000))` | `BTstack.bleConnect(` |
| `rp_ble_disconnect` | Statement | DEV_VAR(field_variable) | `rp_ble_disconnect($bleDevice)` | `BTstack.bleDisconnect(` |
