...
| `multistepper_positions_array` | Value | INPUT0(input_value) | `multistepper_positions_array(math_number(0))` | Dynamic code |
...
1. **Variable**: `accelstepper_setup("varName", ...)` creates variable `$varName`; reference it later with `variables_get($varName)`.
