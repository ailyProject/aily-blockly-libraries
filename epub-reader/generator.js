...
Arduino.forBlock['epub_reader_load_sd_font'] = function(block, generator) {
  ...
  let bootFn = '';
  bootFn += 'static int bootCX, bootCY, bootBX, bootBY, bootBW, bootBH;\n';
  bootFn += 'static void bootDrawLoading(int pct, uint16_t color, const char* msg) {\n';
  bootFn += '  bootCX = tft.width() / 2; bootCY = tft.height() / 2;\n';
  bootFn += '  bootBW = tft.width() - 60; if (bootBW > 200) bootBW = 200; bootBH = 14;\n';
  bootFn += '  bootBX = bootCX - bootBW / 2; bootBY = bootCY + 6;\n';
  bootFn += '  tft.fillRect(bootBX + 2, bootBY + 2, (bootBW - 4) * pct / 100, bootBH - 4, color);\n';
  bootFn += '  tft.fillRect(bootBX + 2 + (bootBW - 4) * pct / 100, bootBY + 2, bootBW - 4 - (bootBW - 4) * pct / 100, bootBH - 4, 0x0000);\n';
  bootFn += '  tft.setTextDatum(4);\n';
  bootFn += '  tft.setTextColor(0xFFFF, 0x0000);\n';
  bootFn += '  tft.fillRect(0, bootBY + bootBH + 6, tft.width(), 16, 0x0000);\n';
  bootFn += '  tft.drawString(msg, bootCX, bootBY + bootBH + 14);\n';
...
Arduino.forBlock['epub_reader_show_bookshelf'] = function(block, generator) {
  ...
  fn += '    tft.drawRoundRect(ex, ey, ew, ew * 4 / 3, 4, 0x4208);\n';
  fn += '    tft.drawFastVLine(ex + ew / 5, ey + 6, ew * 4 / 3 - 12, 0x4208);\n';
  fn += '    const char* es = useSdf ? "书架还空着" : "No books";\n';
  fn += '    int tw2 = useSdf ? uiF->measureString(es) : tft.textWidth(es);\n';
  fn += '    if (useSdf) uiF->drawString((pageW - tw2) / 2, ey + ew * 4 / 3 + 8, es, 0x8410);\n';
  fn += '    else { tft.setTextColor(0x8410); tft.setCursor((pageW - tw2) / 2, ey + ew * 4 / 3 + 8); tft.print(es); }\n';
  fn += '    return;\n';
  fn += '  }\n';
  ...
  fn += '  if (useSdf) { uiF->drawString(6, hdrTy, hdrStr.c_str(), 0x07FF); uiF->drawString(6 + hdrW, hdrTy, cntStr.c_str(), 0x8410); }\n';
  fn += '  else { tft.setTextColor(0x07FF); tft.setCursor(6, hdrTy); tft.print(hdrStr); tft.setTextColor(0x8410); tft.setCursor(6 + hdrW, hdrTy); tft.print(cntStr); }\n';
  fn += '  int pad = 2;\n';
  ...
  fn += '    if (isSel) {\n';
  fn += '      tft.fillRoundRect(1, y - 1, pageW - 6, lh - 2, 4, 0x017A);\n';
  fn += '      tft.fillRect(5, y - 1, pageW - 14, (lh - 2) / 2, 0x01B9);\n';
  fn += '      tft.fillRoundRect(1, y - 1, 3, lh - 2, 1, 0x07FF);\n';
  fn += '      tft.drawFastVLine(pageW - 7, y + 1, lh - 4, 0x07FF);\n';
  fn += '    }\n';
  fn += '    char numStr[8]; snprintf(numStr, 8, "%d.", idx + 1);\n';
  fn += '    uint16_t numColor = isSel ? 0x07FF : 0x8410;\n';
  ...
  fn += '    }\n';
  fn += '    uint16_t fg = isSel ? 0xFFFF : 0xC618;\n';
  ...
  fn += '    tft.fillRoundRect(barX - 1, barAreaY, 2, barAreaFullH, 1, 0x4208);\n';
  fn += '    tft.fillRoundRect(barX - 2, thumbY, 4, thumbH, 2, 0x07FF);\n';
  fn += '  }\n';
  ...
  fn += '  uint16_t bsFc[4] = {0x07FF, 0x8410, 0x07FF, 0x8410};\n';
...
Arduino.forBlock['epub_reader_show_history'] = function(block, generator) {
  ...
  fn += '  if (useSdf) { uiF->drawString(6, hdrTy, hdrT, 0x07FF); uiF->drawString(6 + hdrTW, hdrTy, pageStr.c_str(), 0x8410); }\n';
  fn += '  else { d->setTextColor(0x07FF); d->setCursor(6, hdrTy); d->print(hdrT); d->setTextColor(0x8410); d->setCursor(6 + hdrTW, hdrTy); d->print(pageStr); }\n';
  fn += '  if (total <= 0) {\n';
  ...
  fn += '    if (useSdf) uiF->drawString((pageW - ew) / 2, ey, es, 0x8410);\n';
  fn += '    else { d->setTextColor(0x8410); d->setCursor((pageW - ew) / 2, ey); d->print(es); }\n';
  fn += '  }\n';
  fn += '  for (int i = 0; i < maxShow; i++) {\n';
  fn += '    int idx = topItem + i;\n';
  fn += '    int y = hdrH + 2 + i * lh;\n';
  ...
  fn += '    if (isSel) { d->fillRoundRect(1, y - 1, pageW - 6, lh - 2, 4, 0x017A); d->fillRect(5, y - 1, pageW - 14, (lh - 2) / 2, 0x01B9); d->fillRoundRect(1, y - 1, 3, lh - 2, 1, 0x07FF); d->drawFastVLine(pageW - 7, y + 1, lh - 4, 0x07FF); }\n';
  ...
  fn += '    if (ew > maxPx) {\n';
  fn += '      while (entry.length() > 0) { entry.remove(entry.length() - 1); String te = entry + "..."; int tw = useSdf ? uiF->measureString(te.c_str()) : tft.textWidth(te); if (tw <= maxPx) break; }\n';
  fn += '      entry += "...";\n';
  fn += '    }\n';
  fn += '    int tot = epubReader.histChapterTotal(idx);\n';
  fn += '    String prog = (tot > 0) ? (String(epubReader.histChapter(idx) + 1) + "/" + String(tot)) : String("-");\n';
  fn += '    uint16_t fg = isSel ? 0xFFFF : 0xC618;\n';
  fn += '    uint16_t pc = isSel ? 0x07FF : 0x8410;\n';
  ...
  fn += '    d->fillRoundRect(pageW - 5, barAreaY, 2, barAreaFullH, 1, 0x4208);\n';
  fn += '    d->fillRoundRect(pageW - 6, thumbY, 4, thumbH, 2, 0x07FF);\n';
  fn += '  }\n';
  ...
  fn += '  uint16_t hFc[6] = {0x07FF, 0x8410, 0x07FF, 0x8410, 0x07FF, 0x8410};\n';
...
Arduino.forBlock['epub_reader_show_toc'] = function(block, generator) {
  ...
  fn += '  uint16_t tocFc[8] = {0x07FF, 0x8410, 0x07FF, 0x8410, 0x07FF, 0x8410, 0x07FF, 0x8410};\n';
...
Arduino.forBlock['epub_reader_show_page'] = function(block, generator) {
  ...
  fn += '    if (ph3 > 44) ph3 = 44;\n';
  fn += '    if (ph3 > 12) {\n';
  fn += '      tft.drawRoundRect(2, yPos + 2, maxW - 4, ph3 - 4, 4, 0x4208);\n';
  fn += '      tft.drawLine(4, yPos + 4, maxW - 6, yPos + ph3 - 4, 0x1082);\n';
  fn += '      tft.drawLine(maxW - 6, yPos + 4, 4, yPos + ph3 - 4, 0x1082);\n';
  fn += '      if (sdFont.isLoaded()) { const char* pl = "暂不支持PNG"; int pw2 = sdFont.measureString(pl); sdFont.drawString((maxW - pw2) / 2, yPos + ph3 / 2 - sdFont.getCharHeight() / 2, pl, 0x8410); }\n';
  fn += '      else { tft.setTextColor(0x8410); tft.setTextSize(1); tft.setCursor((maxW - 96) / 2, yPos + ph3 / 2 - 4); tft.print("PNG unsupported"); }\n';
  fn += '    }\n';
  fn += '    return ph3;\n';
  fn += '  }\n';
  ...
  fn += '    if (ph2 > 12) {\n';
  fn += '      tft.drawRoundRect(2, yPos + 2, maxW - 4, ph2 - 4, 4, 0x4208);\n';
  fn += '      tft.drawLine(4, yPos + 4, maxW - 6, yPos + ph2 - 4, 0x1082);\n';
  fn += '      tft.drawLine(maxW - 6, yPos + 4, 4, yPos + ph2 - 4, 0x1082);\n';
  fn += '      if (sdFont.isLoaded()) { const char* pl = "加载中"; int pw2 = sdFont.measureString(pl); sdFont.drawString((maxW - pw2) / 2, yPos + ph2 / 2 - sdFont.getCharHeight() / 2, pl, 0x8410); }\n';
  fn += '      else { tft.setTextColor(0x8410); tft.setTextSize(1); tft.setCursor((maxW - 42) / 2, yPos + ph2 / 2 - 4); tft.print("Loading"); }\n';
  fn += '    }\n';
  ...
  fn += '  epubPanel(&tft, 2, 1, pageW - 4, hdrH - 3);\n';
  fn += '  int hdrTy = 1 + (hdrH - 3 - fh) / 2;\n';
  fn += '  epubDrawTextLine(8, hdrTy, chapStr.c_str(), 0x07FF, useSdFont);\n';
  fn += '  if (titleStr.length() > 0) {\n';
  fn += '    int chapW = useSdFont ? sdFont.measureString(chapStr.c_str()) : tft.textWidth(chapStr);\n';
  fn += '    epubDrawTextLine(8 + chapW, hdrTy, (String(" ") + titleStr).c_str(), 0xC618, useSdFont);\n';
  fn += '  }\n';
  fn += '  int totalChapters = epubReader.getChapterCount();\n';
  ...
  fn += '    if (fillW > 4) tft.drawFastHLine(barX + 2, barY + 1, fillW - 3, 0x39E7);\n';
  fn += '  }\n';
  ...
  fn += '  epubDrawTextLine(pageW - fw, ftrTy, footer.c_str(), 0x07FF, useSdFont);\n';
  ...
  fn += '    epubDrawTextLine(pctX, ftrTy, pctStr, 0x8410, useSdFont);\n';
  fn += '  }\n';
  fn += '}\n';
...
function addCoverFunctions(generator) {
  ...
  fn += '  g_covTx = tx;\n';
  fn += '  tft.fillRoundRect(px + 3, py + 3, pw, ph, 6, 0x1082);\n';
  fn += '  tft.fillRoundRect(px, py, pw, ph, 6, 0x0000);\n';
  fn += '  tft.drawRoundRect(px, py, pw, ph, 6, 0x4208);\n';
  fn += '  tft.drawFastHLine(px + 5, py + 1, pw - 10, 0x39E7);\n';
  fn += '  tft.drawFastHLine(px + 5, py + ph - 2, pw - 10, 0x1082);\n';
  fn += '  if (useSdf) sdFont.drawString(tx, py + 6, zh, 0xFFFF);\n';
  fn += '  else { tft.setTextColor(0xFFFF, 0x0000); tft.setTextSize(1); tft.setCursor(tx, py + 6 + (titleH - 8) / 2); tft.print("Cover"); }\n';
  fn += '  tft.drawFastHLine(px + 8, py + 6 + titleH + 3, pw - 16, 0x5800);\n';
  fn += '}\n';
  ...
  fn += '  tft.setTextColor(0x8410, 0x0000); tft.setTextSize(1);\n';
  ...
  fn += '  tft.fillRoundRect(barX, barY, barW, barH, 4, 0x2104);\n';
  fn += '  tft.drawRoundRect(barX, barY, barW, barH, 4, 0x8410);\n';
  ...
  fn += '  if (fillW > 4) tft.drawFastHLine(barX + 2, barY + 1, fillW - 3, 0xFCA8);\n';
  fn += '}\n';
