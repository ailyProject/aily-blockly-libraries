...
这份文档描述“初始化OTA”块生成的固件行为，以及PC端IDE需要实现的监听、状态显示和上传流程。

## 固件侧固定行为

用户放置`初始化OTA`块后，生成代码会做以下事情：
...
用户工程必须先连接WiFi，再执行`初始化OTA`。如果设备未连接WiFi，`ready`事件中的IP可能是`0.0.0.0`，PC端应视为不可上传。
...
3. 上传前确认当前工程已包含`初始化OTA`块，否则下次上传后设备可能失去OTA能力。
